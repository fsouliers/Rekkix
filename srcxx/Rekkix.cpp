...
/*!
 * Constructor #1 : Only build the main window
 * \param[in]    parent     QApplication running this window
 */
...
/*!
 * Constructor #2 : a configuration filename has been given as an argument, it is possible
 * to load it now.
 * \param[in]    parent     QApplication running this window
 * \param[in]    p_filename Full path of the file that should be loaded
 */
...
/*!
 * Common elements of initialization
 */
void
Rekkix::__common_init()
{
	// tab config
	...
	this->tv_upstreamDocs->setModel(&__currentlyDisplayedUptreamDocsModel) ;
...
void
Rekkix::__loadFileAndInitGui(const char* p_filename)
{
...
void
Rekkix::slt_loadConfigurationFile()
{
	QString filename ;

	filename = QFileDialog::getOpenFileName( this
	                                         , trUtf8("Choix du fichier à ouvrir")
	                                         , "."
	                                         , trUtf8("Fichiers XML (*.xml);;Tous (*.*)") ) ;
...
Rekkix::slt_startAnalysis()
{
	...
	for (it = files.begin(), i=1 ; it != files.end() ; ++it, ++i)
	{
		...
		progbar_value = 84*i/nbFiles + 1 ; // dumb calculation just to ensure the value will alway be in [1;85]
...
void
Rekkix::slt_fileCoverageSummary_selected(QModelIndex /*p_index*/)
{
	// clear all the models of the other views
	__currentlyDisplayedUptreamDocsModel.clear() ;
	...
	if (ism->hasSelection())
	{
		foreach(QModelIndex mi, ism->selectedRows())
		{
			...
			if (f)
			{
				__currentlyDisplayedUptreamDocsModel.addDocumentSet(f->getUpstreamDocuments()) ;
		...
		__currentlyDisplayedUptreamDocsModel.refresh() ;
...
void
Rekkix::slt_generateReports()
{
	QDateTime reportTimestamp = QDateTime::currentDateTime() ;

	foreach(ModelConfiguration::XmlAttributesMap_t outFileDescription, __cnfModel.getOutputFiles())
	{
	...
	QMessageBox::information(NULL, QObject::trUtf8("Génération du rapport terminée"), QObject::trUtf8("Fin de génération des rapports"), QMessageBox::Ok) ;
}

const QString
Rekkix::__getReportSummaryTable(const QString& p_writer, const QString& p_delimiter) const
{
	FactoryReportBaseString fact ;
...
const QString
Rekkix::__getReportErrorsTable(const QString& p_writer, const QString& p_delimiter) const
{
	FactoryReportBaseString fact ;
