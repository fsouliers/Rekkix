...
int
ModelConfiguration::columnCount(const QModelIndex & /*parent*/) const
{
...
int
ModelConfiguration::rowCount(const QModelIndex & /*parent*/) const
{
...
QVariant
ModelConfiguration::data(const QModelIndex &index, int role) const
{
	QVariant def_value ;
...
QVariant
ModelConfiguration::headerData(int section, Qt::Orientation orientation, int role) const
{
	if (role == Qt::DisplayRole)
	{
...
QModelIndex
ModelConfiguration::index(int row, int column, const QModelIndex & /*parent*/) const
{
...
QModelIndex
ModelConfiguration::parent(const QModelIndex & /*index*/) const
{
	return(QModelIndex()) ;
}
...
void
ModelConfiguration::setFile(const char* p_filepath, ModelConfigurationErrorsRef errModel)
{
	...
	while (!xstr.atEnd())
	{
		if (xstr.isStartElement())
		{
			if (xstr.name().toString() == XML_NODE_FILE)
			{
				...
				qDebug() << "ModelConfiguration::setFile : checking consistency " ;

				if (! __hasAReqFileConsistecyErrors(newItem, errModel))
				{
			...
			else if (xstr.name().toString() == XML_NODE_OUTPUT)
			{
				...
				if (! __hasAnOutputFileConsistecyErrors(newItem, errModel))
				{
...
bool
ModelConfiguration::__hasAReqFileConsistecyErrors(const XmlAttributesMap_t& configured_file, ModelConfigurationErrorsRef errModel)
{
...
bool
ModelConfiguration::__hasAnOutputFileConsistecyErrors(const XmlAttributesMap_t& configured_file, ModelConfigurationErrorsRef errModel)
{
	bool retVal = true ; // errors by default
	ModelConfigurationErrors::error_t e ;
	QVector<ModelConfigurationErrors::error_t> errors ;
