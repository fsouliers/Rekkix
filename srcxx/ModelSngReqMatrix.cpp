...
void
ModelSngReqMatrix::addRequirementFile(IRequirementFilePtr p)
{
...
{
	bool retval = false ; // by default, the methon fails
	...
	if (p_r.getState() != Requirement::Defined)
	{
		AnalysisError e(AnalysisError::ERROR
		                ...
		                , file_id
		                , QObject::trUtf8("Ajout en non défini d'une exigence qui devrait être définie --> erreur dans l'analyseur de fichier")) ;
...
void
ModelSngReqMatrix::addExpectedCompositeRequirement(const QString& file_id, QString& p_parentId, RequirementRef p_r)
{
...
void
ModelSngReqMatrix::addExpectedCoveredRequirement(const QString& file_id, QString& p_reqId, RequirementRef p_coveredReq)
{
	// The method has been called while there is no consistent «current requirement» parsing
	// the requirement file
...
void
ModelSngReqMatrix::computeCoverage()
{
...
int
ModelSngReqMatrix::columnCount(const QModelIndex & /*parent*/) const
{
...
int
ModelSngReqMatrix::rowCount(const QModelIndex & /*parent*/) const
{
...
QVariant
ModelSngReqMatrix::data(const QModelIndex &index, int role) const
{
	QVariant retVal ;
...
QVariant
ModelSngReqMatrix::headerData(int section, Qt::Orientation orientation, int role) const
{
	if (role == Qt::DisplayRole)
	{
...
QModelIndex
ModelSngReqMatrix::index(int row, int column, const QModelIndex & /*parent*/) const
{
...
QModelIndex
ModelSngReqMatrix::parent(const QModelIndex & /*index*/) const
{
	return(QModelIndex()) ;
}
...
IRequirementFilePtr
ModelSngReqMatrix::getRequirementFile(const QString& p_fileID ) const
{
...
const QString
ModelSngReqMatrix::getReportFileSummaryTable(const QString& p_writer, const QString& p_delimiter) const
{
	...
	for (it = __filesByFileId.constBegin() ; it != __filesByFileId.constEnd() ; ++it)
	{
		...
		{
			newline.replace("REKKIX_SUMMARY_TABLE_FILE_ID", QString("<a href=\"#%1\">%2</a>").arg(file_id).arg(file_id)) ;
		}
		else
		{
		...
		newline.replace("REKKIX_SUMMARY_TABLE_FILE_NB_REQS", QString("%1").arg(__reqsByFileId[file_id].count())) ;
		...
		if (nbErrors > 0)
		{
			newline.replace("REKKIX_SUMMARY_TABLE_ROW_COLOR", STR_CSS_INVALID_ROW_CLASS_ATTR) ; // in case of CSV, nothing to replace -> nothing happens
		}
...
const QString
ModelSngReqMatrix::getReportFileDetails(const QString& p_writer, const QString& p_delimiter) const
{
	FactoryReportBaseString fact ;
	...
	for (it = __filesByFileId.constBegin() ; it !=  __filesByFileId.constEnd() ; ++it)
	{
		...
		newfile.replace("REKKIX_FILE_DOC_COVERAGE_CONTENT", __getReportFileDetailsDocCoverage(file_id, p_writer, p_delimiter)) ;
		if (it.value()->mustHaveDownstreamDocuments())
		{
			newfile.replace("REKKIX_FILE_REQ_COVERAGE_CONTENT", __getReportFileDetailsReqCoverage(file_id, p_writer, p_delimiter)) ;
		}
		else
		{
		...
		{
			newfile.replace("REKKIX_FILE_REQ_REVCOV_CONTENT", __getReportFileDetailsRevCov(file_id, p_writer, p_delimiter)) ;
		}
		else
		{
...
const QString
ModelSngReqMatrix::__getReportFileDetailsDocCoverage(const QString& file_id,const QString& p_writer, const QString& p_delimiter) const
{
	FactoryReportBaseString fact ;
...
const QString
ModelSngReqMatrix::__getReportFileDetailsReqCoverage(const QString& file_id,const QString& p_writer, const QString& p_delimiter) const
{
	QString result = "" ;
	if (__filesByFileId[file_id]->mustHaveDownstreamDocuments())
	{
		...
		// Data independent values
		result.replace("REKKIX_FILE_DEFINED_REQS_WITH_COMPOSITE_AND_COVERAGE_TITLE", QObject::trUtf8("Couverture du fichier")) ;
		...
		result.replace("REKKIX_FILE_COV_REQ_COVERED_BY_LOCATION_ID_TITLE", QObject::trUtf8("Qui est définie dans")) ;
...
const QString
ModelSngReqMatrix::__getReportFileDetailsRevCov(const QString& file_id,const QString& p_writer, const QString& p_delimiter) const
{
	FactoryReportBaseString fact ;
	...
	// Data independent values
	s_reqRevCov.replace("REKKIX_FILE_DEFINED_REQS_WITH_COMPOSITE_AND_REVERSE_COVERAGE_TITLE", QObject::trUtf8("Couverture réalisée par le fichier")) ;
	...
	s_reqRevCov.replace("REKKIX_FILE_REVCOV_COVERING_LOCATION_ID_TITLE", QObject::trUtf8("Qui est définie dans")) ;
