...
static const char* DOCUMENT_FILE = "word/document.xml" ; //<! XML file of the docx zip containing the document data (ie in witch the requirements can be found)

static const int ZIP_BUFFER_SIZE = 1048576; //!< Size of a 1MB array of char
static char ZIP_BUFFER[ZIP_BUFFER_SIZE] ;

static const QString DOCX_XML_PARAGRAPH_NODE = "p" ;
static const QString DOCX_XML_TEXT_NODE = "t" ;

RequirementFile_docx::RequirementFile_docx(ModelConfiguration::XmlAttributesMap_t& p_cnfFile)
...
void
RequirementFile_docx::__readTextDataFromZippedFormat(QString* p_textData)
{
	int err;
	struct zip* zipfile = zip_open (_cnfFile[ModelConfiguration::REQFILE_ATTR_PATH].toStdString().c_str (), 0, &err);
	if (!zipfile)
	{
...
void
RequirementFile_docx::parseFile()
{
	...
	while (!xstr.atEnd())
	{
		...
		// Now the data string contains a paragraph of the document
		// If the regex match as a new requirement, it must be stored in the global requirement matrix
		...
		if (req_m.hasMatch())
		{
			...
			// If the requirement has already been defined, the global Matrix handles itself
			// the error log
		...
		// requirement matrix as expected requirements (if not already defined, else they have to be completed)
