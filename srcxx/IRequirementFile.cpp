...
IRequirementFile::IRequirementFile(ModelConfiguration::XmlAttributesMap_t& p_cnfFile)
	: _nbReqs(0)
	, _sumOfCov(0.0)
	, _avgCoverage(0.0)
	, _cnfFile(p_cnfFile)
...
void
IRequirementFile::addOneMoreRequirementCoverage(double p_coverage)
{
	_nbReqs += 1 ;
...
void
IRequirementFile::addUpstreamDocument (const QString& p_fileId, IRequirementFile* p_file)
{
	...
	if (_upstreamDocs.contains(p_fileId)) return ;

	_upstreamDocs.insert(p_fileId, p_file) ;
}

void
IRequirementFile::addDownstreamDocument (const QString& p_fileId, IRequirementFile* p_file)
{
	...
	if (_downstreamDocs.contains(p_fileId)) return ;
